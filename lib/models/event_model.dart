...
class EventModel {
  ...
  static EventModel fromMap(Map<String, dynamic> data) {
    return EventModel(
      ...
      eventCreationTime: (data[EVENT_CREATED_AT_KEY] as Timestamp).toDate(),
      eventHost: data[EVENT_HOST_DATA_KEY],
      isEventActive: data[EVENT_ACTIVE_KEY],
      eventInvites: (data[EVENT_INVITED_USERS_KEY] as Iterable).where((val) => val != null).map((val) => val as UserModel).toList(),
      eventAccepts: (data[EVENT_ACCEPTED_USERS_KEY] as Iterable).where((val) => val != null).map((val) => val as UserModel).toList(),
      eventDeclines: (data[EVENT_DECLINED_USERS_KEY] as Iterable).where((val) => val != null).map((val) => val as UserModel).toList(),
      eventUndecided: (data[EVENT_TENTATIVE_USERS_KEY] as Iterable).where((val) => val != null).map((val) => val as UserModel).toList(),
      eventDescription: data[EVENT_DESCRIPTION_KEY],
      eventTimeStart: (data[EVENT_START_TIME_KEY] as Timestamp).toDate(),
      eventTimeEnd: (data[EVENT_END_TIME_KEY] as Timestamp).toDate(),
